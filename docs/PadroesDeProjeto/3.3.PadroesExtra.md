...
Esse padrão de projeto é caracterizado principalmente por separar processamentos, gerencia de dados e funções de apresentação fisica e logicamente separadas. Dessa forma, o projeto se torna mais fácil de gerenciar, mais fácil de identificar os bugs e as mudanças realizadas em uma parte do projeto não afetaria as outras. 

Usualmente, temos três diferentes camadas nesse padrão: 
- Camada de apresentação: É voltado para interface do usuário, é o que vai traduzir e apresentar ao usuário o sistema.
...
No nosso projeto, iremos utilizar o N-tier em conjunto com o [MVC](/3.3.PadroesExtra?id=_331-mvc-model-modelo-view-visão-controller-controle). Nesse sentido, o MVC será utilizado como padrão dentro das camadas (a View na camada apresentação e a Controller e a Model na camada de lógica), enquanto o N-tier será utilizado como arquitetura geral. Pois dessa forma, podemos aproveitar flexibilidade e reaproveitamento de código do MVC e também a melhor gerencia de projeto do N-tier. 
